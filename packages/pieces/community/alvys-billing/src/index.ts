import { createPiece } from '@activepieces/pieces-framework';
import { PieceCategory } from '@activepieces/shared';

import { alvysBillingAuth } from './lib/auth';
import { draftCustomerInvoiceAction } from './lib/actions/draft-customer-invoice';
import { draftDriverSettlementAction } from './lib/actions/draft-driver-settlement';
import { sendEdi210Action } from './lib/actions/send-edi-210';

export { alvysBillingAuth };

export const alvysBilling = createPiece({
  displayName: 'Alvys Billing',
  description:
    'AR/AP and settlement automations — draft invoices, apply payments, send EDI 210, generate driver settlements.',
  auth: alvysBillingAuth,
  minimumSupportedRelease: '0.36.1',
  logoUrl: 'data:image/png;base64,',
  authors: ['alvys'],
  categories: [PieceCategory.ACCOUNTING],
  actions: [draftCustomerInvoiceAction, draftDriverSettlementAction, sendEdi210Action],
  triggers: [],
});
