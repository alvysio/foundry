import { createPiece } from '@activepieces/pieces-framework';
import { PieceCategory } from '@activepieces/shared';

import { alvysDispatchAuth } from './lib/auth';
import { assignCarrierToLoadAction } from './lib/actions/assign-carrier-to-load';
import { postLoadStatusAction } from './lib/actions/post-load-status';

export { alvysDispatchAuth };

export const alvysDispatch = createPiece({
  displayName: 'Alvys Dispatch',
  description:
    'Dispatch automations on Alvys — assign carriers, post status updates, manage stops, send check calls.',
  auth: alvysDispatchAuth,
  minimumSupportedRelease: '0.36.1',
  logoUrl: 'data:image/png;base64,',
  authors: ['alvys'],
  categories: [PieceCategory.BUSINESS_INTELLIGENCE],
  actions: [assignCarrierToLoadAction, postLoadStatusAction],
  triggers: [],
});
