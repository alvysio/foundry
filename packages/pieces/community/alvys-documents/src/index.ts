import { createPiece } from '@activepieces/pieces-framework';
import { PieceCategory } from '@activepieces/shared';

import { alvysDocumentsAuth } from './lib/auth';
import { extractDocumentAction } from './lib/actions/extract-document';
import { buildExtractPipelineAction } from './lib/actions/build-extract-pipeline';

export { alvysDocumentsAuth };

export const alvysDocuments = createPiece({
  displayName: 'Alvys Document Intelligence',
  description:
    'Extract structured data from transportation documents (POD, ratecon, BOL, COI, MVR, IFTA). Powered by bem.ai with Alvys-managed schemas and tenant custom-reference injection per entity (load, trip, driver, truck, carrier).',
  auth: alvysDocumentsAuth,
  minimumSupportedRelease: '0.36.1',
  logoUrl: 'data:image/png;base64,',
  authors: ['alvys'],
  categories: [PieceCategory.ARTIFICIAL_INTELLIGENCE],
  actions: [extractDocumentAction, buildExtractPipelineAction],
  triggers: [],
});
